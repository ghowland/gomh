...
from gomh_oo_01_code import collision
...
class Actor:
  ...
  def GetDistanceToActor(self, actor):
    dist = math.sqrt((actor.pos.x - self.pos.x)**2 + (actor.pos.y - self.pos.y)**2)
    return dist
  ...
  def Update(self):
    ...
    # Fall, if you can
    if actor.jump == 0:
      (fall_pos, collision_actor) = collision.MovePosCollide(self.config, actor, Vector2(0, actor.fall), self.config.ACTORS, self.config.scene_mask)
    ...
    if actor.jump > 0:
      ...
      for count in range(0, int(actor.jump)):
        (jump_pos, collision_actor) = collision.MovePosCollide(self.config, actor, Vector2(0, -1), self.config.ACTORS, self.config.scene_mask)
    ...
    (player_below_test_pos, collision_actor) = collision.MovePosCollide(self.config, actor, Vector2(0, 1), self.config.ACTORS, self.config.scene_mask)
    ...
    # Test Right
    (damage_test_pos, collision_actor) = collision.MovePosCollide(self.config, actor, Vector2(1, 0), self.config.ACTORS, self.config.scene_mask, scene_obstacle_color=(255,66,246))
  ...
  def UpdateNPC(self):
    ...
    if dist > self.image_size.x * 3:
      target_far = True
    else:
      target_far = False
    
    # If above, by more than half your size, Move Away
    if self.pos.y > target_actor.pos.y:
      #print 'Actor Above Self: %s ----- %s' % (self, target_actor)
      target_above_self = True

    # If above, by more than half your size, Move Away
    if self.pos.y < target_actor.pos.y + target_actor.image_size.y:
    ...
    if target_above_self and not target_far:
      if self.pos.x > target_actor.pos.x:
        target_move = Vector2(self.speed, 0)
      else:
        target_move = Vector2(-self.speed, 0)
  ...
  def Walk(self, move):
    ...
    # Else, character collision, push them
      ...
      push = Vector2(move.x * 2, move.y * 2)
  ...
  def Respawn(self):
    ...
    while not found_good_respawn_point:
      respawn_x = random.randint(0, self.config.scene.get_width() - self.image_size.x)
      self.pos = Vector2(respawn_x, 130)
      
      #TODO(g): Could be colliding with scene, in a different scene than the test one
      [target_pos, collision_actor] = collision.MovePosCollide(self.config, self, Vector2(0,0), self.config.ACTORS, self.config.scene_mask)
  ...
  def Jump(self):
    ...
    [ground_test_pos, collision_actor] = collision.MovePosCollide(self.config, self, Vector2(0, 1), self.config.ACTORS, self.config.scene_mask)
    # If we are free to jump
    if ground_test_pos == self.pos and self.jump == 0:
      # Test if there is an actor (or obstacle) directly above us
      [actor_on_head_test_pos, collision_actor] = collision.MovePosCollide(self.config, self, Vector2(0, -1), self.config.ACTORS, self.config.scene_mask)
